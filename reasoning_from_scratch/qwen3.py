...
class Qwen3Model(nn.Module):
    ...
    def forward(self, input_ids, cache=None):
        ...
        x = self.tok_emb(input_ids)  # embed IDs -> [batch_size, seq_len, emb_dim]
        num_tokens = x.shape[1]

        if cache is not None:
            # Continuing from cached sequence
            pos_start = self.current_pos
            pos_end = pos_start + num_tokens
            self.current_pos = pos_end
            total_len = pos_end

            # Build mask with shape (num_tokens, total_len)
            mask = torch.triu(
                torch.ones(total_len, total_len, device=x.device, dtype=torch.bool),
                diagonal=1
            )[pos_start:pos_end, :total_len]
        else:
            # Fresh sequence
            pos_start = 0
        ...
        # Expand mask for batch and heads: [1, 1, num_tokens, total_len]
        mask = mask[None, None, :, :]
        ...
        for i, block in enumerate(self.trf_blocks):
            ...
            new_caches.append(new_blk_cache)
    ...
    def reset_kv_cache(self):
        """
        Reset KV cache and mask state for a fresh conversation.
        """
        self.current_pos = 0
        # If you have a KVCache object, clear it too
        if hasattr(self, "kv_cache") and self.kv_cache is not None:
            self.kv_cache.reset()
        # Force mask buffers to reset by re‑registering cos/sin
        head_dim = self.cfg["head_dim"] if self.cfg["head_dim"] else self.cfg["emb_dim"] // self.cfg["n_heads"]
        cos, sin = compute_rope_params(
            head_dim=head_dim,
...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(self, x, mask, cos, sin, start_pos=0, cache=None):
        ...
        # 🔹 Fix: align mask dimensions before applying
        if mask is not None:
            mask = mask[..., :attn_scores.shape[-2], :attn_scores.shape[-1]]
...
def download_qwen3_small(kind="base", tokenizer_only=False, out_dir="."):
    ...
    for key in targets:
        ...
        if local_path.is_file():
            print(f"✓ Found {fname} locally in {out_dir}, skipping download.")
            continue  # <-- skip internet if file exists
